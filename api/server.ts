...
class GoogleService {
  ...
  async deleteRow(spreadsheetId: string, range: string, idColumn: string, idValue: string) {
    ...
    if (rowIndex === -1) throw new Error('Không tìm thấy bản ghi để xóa');

    // Xóa hàng bằng BatchUpdate (Xóa thật sự hàng đó khỏi Sheet)
...
app.post('/api/db/:sheetName', async (req: any, res) => {
  try {
    const { google: googleSvc } = getServices(req);
    await googleSvc.appendRow(process.env.GOOGLE_SHEET_ID!, req.params.sheetName, req.body);
    res.json({ success: true });
  } catch (err: any) { res.status(500).json({ error: err.message }); }
});
...
app.post('/api/report/generate', async (req: any, res) => {
  try {
    ...
    let rowData;
    try {
      const url = `https://api.appsheet.com/api/v2/apps/${appsheetSvc.appId}/tables/${tableName}/Action`;
      const findResponse = await axios.post(url, { 
        Action: 'Find', 
        Properties: { Locale: 'vi-VN' }, 
        Rows: [{ [keyColumn]: rowId }] 
      }, { 
        headers: { ApplicationAccessKey: appsheetSvc.accessKey } 
      });
      
      const parentRow = findResponse.data && findResponse.data[0];
      ...
      return res.status(400).json({ success: false, error: `Lỗi AppSheet: ${e.message}. Hãy kiểm tra cấu hình bảng.` });
    }
